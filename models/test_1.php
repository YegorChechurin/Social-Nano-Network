        ...
        <div class="col-sm-3">
            <div id="chats_wrapper">
              <?php foreach($chats as $key=>$value): ?>
                <div class="chat_header" onclick="display_chat(<?=$value['partner_id']?>); catch_partner(<?=$value['partner_id']?>)">
                    <?=$value['last_mes_auth_name'].': '.$value['last_mes_text']?> 
                </div>
              <?php endforeach; ?>
            </div>
	   </div>
...
<script type="text/javascript">
	...
	function form_chat_header(chat) {
		document.getElementById("chats_wrapper").innerHTML += '<div class="chat_header" onclick="display_chat('+chat.partner_id+')">'+'<b>'+chat.partner_name+'</b><br>'+chat.last_mes_auth_name+': '+chat.last_mes_text+'</div>';
	}
	...
	function display_message(message) {
		if (message.sender_id==id || message.recipient_id==id) {
		    ...
		    if (message.sender_id == user_id) {
                ...
                document.getElementById("mes").innerHTML += '<div class="message_inlet" id="m'+n+'"><b>Your friend:</b> '+message.message+'</div><br>';
		    }
		} else {
